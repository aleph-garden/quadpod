...
## ADR-15: The documentation site lives in this repository, with bun in the dev shell

The reader-facing documentation at `aleph.garden/quadpod/docs/` is built from `site/`, an
Astro Starlight project beside the crate, and deployed as its own Worker. It reads its pages
from `docs/`, so a document and the code it describes change in the same commit, the rule
[`architecture.md`](architecture.md) sets for itself. The dev shell carries bun for it.

**What it costs.** A second toolchain in a Rust repository, and a `node_modules` beside
`target/`. Nothing in the crate depends on it, and `cargo` never runs it.

**The alternative that lost.** The Markdown here and the site scaffold in the `www`
repository, which builds the landing page. The crate would stay pure Rust, and a change to a
document would reach the site only after a push to a second repository.

**What would reopen it.** A documentation host shared by every Aleph Garden project that
reads each project's `docs/` directly, which would make a scaffold per repository redundant.

<a id="adr-16"></a>

## ADR-16: The configuration reference is generated by a test and committed

`docs/reference/configuration.md` is rendered from the argument definition in
`src/config.rs` by `tests/configuration_reference.rs`, which fails when the committed file
differs. Flags, environment variables and defaults on the page are the binary's own, and the
site build reads a file, so it needs no Rust.

**The alternative that lost.** A hand-written page with a constraint that every flag's name
appears on it. That catches a missing flag and misses a changed default or a renamed
environment variable, which are the changes an operator trips over.

**What would reopen it.** The site build gaining a Rust toolchain for another reason, at
which point it could render the page itself and the committed copy would be redundant.
